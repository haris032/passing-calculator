...
# Main Option: Assignments huin ya nahi
assignment_status = st.selectbox(
    "📊 Were assignments conducted this semester?",
    ["Yes", "No"],
    index=0  # Default "Yes" par set hai
)
...
with col1:
    ...
    # Agar user "Yes" select karega, sirf tab assignments show hongi
...
if assignment_status == "No":
    # Agar assignments nahi huin, to 20% weightage finals mein add ho jayega (Finals = 60%)
    current_total_percentage = quiz_weightage + mid_weightage
    final_weightage = 60.0
    assignment_weightage = 0.0
else:
    # Agar assignments huin, to sessional total 60% ka aur finals 40% ka
...
sessional_limit = 40.0 if assignment_status == "No" else 60.0
